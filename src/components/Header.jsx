...
function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 px-4 pt-4">
      <div className="glass mx-auto flex max-w-7xl items-center rounded-[1.6rem] px-4 py-3 shadow-sm sm:px-6">
        <nav className="hidden flex-1 items-center justify-start gap-7 lg:flex">
          {nav.map((item) => (
            <a
              key={item}
              href={anchorFor(item)}
        ...
        <a
          href="#home"
          className="ml-auto flex items-center justify-end gap-3 lg:flex-1"
        >
